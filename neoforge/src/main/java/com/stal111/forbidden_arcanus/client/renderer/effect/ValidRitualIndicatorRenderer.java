...
public class ValidRitualIndicatorRenderer {
    ...
    private static final SpriteId TEXTURE = MagicCircleRenderer.MAPPER.apply(ForbiddenArcanus.identifier("valid_ritual_indicator"));
    ...
    public void submit(ValidRitualIndicatorRenderState renderState, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
        ...
        nodeCollector.submitModel(this.model, Unit.INSTANCE, poseStack, TEXTURE.renderType(this.model::renderType), renderState.lightCoords, OverlayTexture.NO_OVERLAY, ARGB.colorFromFloat(Math.min(1.0F, easeOutBack(renderState.ageInTicks, 0.0F, 1.0F, ANIMATION_DURATION)), 1.0F, 1.0F, 1.0F), this.sprites.get(TEXTURE), 0, null);
