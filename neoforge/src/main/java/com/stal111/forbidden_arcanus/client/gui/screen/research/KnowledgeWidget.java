...
public class KnowledgeWidget extends AbstractWidget {
    ...
    protected void renderWidget(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.display.getFrame().getFrameTexture(this.locked, this.isHoveredOrFocused()), this.getX() + scrollX, this.getY() + scrollY, 26, 26);
    ...
    private void renderBox(ResourceLocation texture, GuiGraphics guiGraphics, int x, int y, int width, int height) {
        ...
        while (i > 0) {
            ...
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, texture, 120, 20, 45, 0, x2, y, j, height);
            i -= j;
            x2 += j;
        }

        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, texture, 120, 20, 120 - 45, 0, x + width - 45, y, 45, height);
    }
    ...
    private static class UnlockAnimation {
        ...
        public void render(GuiGraphics guiGraphics, int x, int y) {
            ...
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TEXTURES[(int) (this.animationTick / ANIMATION_SPEED)], x - 19, y - 19, 64, 64);
        }
    }
}
