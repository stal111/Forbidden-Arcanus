...
public class ForbiddenArcanusJEIPlugin implements IModPlugin {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        registration.addRecipes(HEPHAESTUS_SMITHING, Minecraft.getInstance().level.registryAccess().registryOrThrow(FARegistries.RITUAL).entrySet().stream().map(Map.Entry::getValue).filter(ritual -> ritual.result() instanceof CreateItemResult).toList());
