...
public class DarkBeaconTileEntity extends TileEntity implements INamedContainerProvider, ITickableTileEntity {
	...
	public static final Effect[][] EFFECTS_LIST = new Effect[][] {
		{ Effects.WEAKNESS, Effects.SLOWNESS },
		{ Effects.WITHER, Effects.POISON },
		{ Effects.NAUSEA, Effects.BLINDNESS }
		};
	...
	private final IIntArray field_213937_n = new IIntArray() {
		public int get(int index) {
			...
			case 1:
				return Effect.getId(DarkBeaconTileEntity.this.effect);
			default:
				return 0;
			}
		}
		...
		public void set(int index, int value) {
			...
			case 1:
				...
				DarkBeaconTileEntity.this.effect = DarkBeaconTileEntity.isBeaconEffect(value);
			}
	...
	public Container createMenu(int i, PlayerInventory inventory, PlayerEntity p_createMenu_3_) {
		return LockableTileEntity.canUnlock(p_createMenu_3_, this.field_213936_m, this.getDisplayName()) ? new DarkBeaconContainer(i, inventory, this.field_213937_n, IWorldPosCallable.of(this.world, this.getPos())) : null;	}
	...
	public void tick() {
		...
		DarkBeaconTileEntity.BeamSegment beacontileentity$beamsegment = this.field_213934_g.isEmpty() ? null
				: this.field_213934_g.get(this.field_213934_g.size() - 1);
			...
			if (afloat != null) {
				if (this.field_213934_g.size() <= 1) {
					...
					if (Arrays.equals(afloat, beacontileentity$beamsegment.colors)) {
						...
						beacontileentity$beamsegment = new DarkBeaconTileEntity.BeamSegment(
				...
				if (beacontileentity$beamsegment == null
						|| blockstate.getOpacity(this.world, blockpos) >= 15 && block != Blocks.BEDROCK) {
		...
		for (int i = 1; i <= 4; this.levels = i++) {
			...
			for (int k = p_213927_1_ - i; k <= p_213927_1_ + i && flag; ++k) {
				for (int l = p_213927_3_ - i; l <= p_213927_3_ + i; ++l) {
					if (!(this.world.getBlockState(new BlockPos(k, j, l)).getBlock() == ModBlocks.arcane_gold_block))
					 {
						flag = false;
						break;
					}
	...
	private void addEffectsToPlayers() {
		if (!this.world.isRemote && this.effect != null) {
			...
			AxisAlignedBB axisalignedbb = (new AxisAlignedBB(this.pos)).grow(d0).expand(0.0D,
			...
			for (PlayerEntity playerentity : list) {
				playerentity.addPotionEffect(new EffectInstance(this.effect, j, 0, true, true));
			}
		}
	}
	...
	public void read(CompoundNBT compound) {
		super.read(compound);
		this.effect = isBeaconEffect(compound.getInt("Effect"));
	...
	public CompoundNBT write(CompoundNBT compound) {
		...
		compound.putInt("Effect", Effect.getId(this.effect));
	...
	public ITextComponent getDisplayName() {
		return (ITextComponent)(this.customName != null ? this.customName : new TranslationTextComponent("container.dark_beacon"));
	}
