...
public class ObsidianSkullTileEntityRenderer extends TileEntityRenderer<ObsidianSkullTileEntity> {
    ...
    public void render(ObsidianSkullTileEntity tileEntity, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        ...
        render(direction, 22.5F * rotation, 0.0F, matrixStack, buffer, combinedLight);
    }

    public static void render(@Nullable Direction direction, float rotation, float animationProgress, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight) {
        matrixStack.push();
        ...
        IVertexBuilder vertexBuilder = buffer.getBuffer(RenderType.getEntityCutoutNoCullZOffset(TEXTURE));
