...
public class ModVegetationPlacements extends DatapackRegistryClass<PlacedFeature> {
    ...
    public void bootstrap(BootstapContext<PlacedFeature> context) {
        PlacementUtils.register(context, YELLOW_ORCHID, ModConfiguredFeatures.YELLOW_ORCHID.getHolder().get(), RarityFilter.onAverageOnceEvery(12), PlacementUtils.HEIGHTMAP, InSquarePlacement.spread(), BiomeFilter.biome());
        PlacementUtils.register(context, CHERRY_TREES_PLAINS, ModVegetationFeatures.CHERRY_TREES_PLAINS, PlacementUtils.countExtra(0, 0.05F, 1), InSquarePlacement.spread(), TREE_THRESHOLD, PlacementUtils.HEIGHTMAP_OCEAN_FLOOR, BlockPredicateFilter.forPredicate(BlockPredicate.wouldSurvive(ModBlocks.CHERRY_SAPLING.get().defaultBlockState(), BlockPos.ZERO)), BiomeFilter.biome());
    }
}
